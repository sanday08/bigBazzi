...
io.on("connection", (socket) => {
  ...
  socket.on("joinAdmin", async ({ adminId }) => {
    ...
    if (user.role == "Admin") {
      ...
      socket.emit("resAdmin", { data: games.position, numbers: numbers.records.splice(0, 5) });
    } else
      socket.emit("res", {
...
getResult = async (stopNum) => {
  ...
  flushAll();
  let numbers = await getLastrecord();
  io.to('adminData').emit("resAdmin", { data: games.position, numbers: numbers.records.splice(0, 5) });
};
